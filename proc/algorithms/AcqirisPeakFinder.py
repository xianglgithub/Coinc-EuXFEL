...
#correction
      ...
      #  print '2: ',len(wf_cal_ind_ind)
        t_cfd_list = []
        for i in range(self.plsNum):
            t_cfd_list.append(np.array([],np.float))
...
#correction
            ...
            ind_tg = next((tg_ind for tg_ind, tg_ele in enumerate(self.triggerT) if t_cfd < tg_ele),0)-1
            
            t_cfd_tg = t_cfd - self.triggerT[ind_tg]
            
            #if t_cfd>self.timerange_low and t_cfd<self.timerange_high:
            t_cfd_list[ind_tg] = np.append(t_cfd_list[ind_tg],t_cfd_tg)
    #            t_cfd_list.append(t_arr[1])
                
      #  print 'changed'
                
        #return t_cfd_list, wf_cal_m_walk, wt[:-self.delay]
        return t_cfd_list
